...
@section('content')
	...
	<div class="bgblue" style="display: inline-block; margin: 0">
		...
		<div id ="add" style="display:inline-table; float:left; margin: 10px;"><div class="upper" style="font-size: 18px"> {{$customer->fullName }} </div>
			{{ $customer->email }}<br />
			{{ $customer->phone_num }}<br />
			<a id="editButton" href ="{{ action('CustomersController@edit', [$customer->id])}}" class="btn addButton" style="padding:0;font-size:12px;color:rgb(233,130,51);height: 20px;">EDIT ACCOUNT
			</a>
		</div>
	</div>
	...
	<div class= "form-group" style="display: inline-flex; margin-bottom: 0;">
		...
		<a href="{{ action('CustomerInsurancesController@create', [$customer->id]) }}" style="text-align: center; margin-top: 1%;" class="btn addButton"> <i>+ Add insurance </i></a>
	</div>
