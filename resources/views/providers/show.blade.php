...
<div class="bgblue" style="display: inline-block;">
	<div style="margin:8px; width:50%; display: inline-flex;">
		...
		<div style="display:inline-table; float:left; margin: 10px 10px 10px 10px;"><div class="upper" style="font-size: 18px"> {{ $provider->name }} </div>
...
<div class= "form-group" style="display: inline-flex; margin-bottom: 0;">
		...
		<a href={{ action('InsurancesController@create', [$provider->id]) }}" style="text-align: center; margin-top: 1%;" class="btn addButton"> <i>+ Add plan </i></a>
	</div>
		...
		<table class="table table-hover cards-table">
			...
			@foreach ($provider->insurances as $insurance)
				<tr id = "override">
					...
					<td id ="buttonContainer"><a href="{{ action('InsurancesController@edit', [$provider->id, $insurance->id]) }}" class="btn btn-primary">Edit</a></td>
				</tr>
			@endforeach
		</table>
	@endunless
